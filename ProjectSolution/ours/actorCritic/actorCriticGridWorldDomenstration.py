...
def learn_from_demonstration(model, demonstration_data):
    states, actions, rewards = demonstration_data
    # Convert states, actions and rewards to tensors
    states = torch.from_numpy(states).float()
    actions = torch.from_numpy(actions).long()
    rewards = torch.from_numpy(rewards).float()
    
    # Pass states through the model to get the actor and critic outputs
    policy, value = model(states)
    # Get the log probabilities of the taken actions
    logprobs = policy.gather(1, actions.view(-1,1))
    
    # Define the loss function for the actor and critic
    actor_loss = -1 * logprobs * rewards
    critic_loss = torch.pow(value - rewards, 2)
